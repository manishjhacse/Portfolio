...
export default function Projects() {
    ...
    return (
        <div className='w-full flex flex-col gap-2'>
            ...
            {/* Work Container */}
            {
                projectDataToShow.map((project) => {
                    return <div key={project.id} className='relative flex gap-8 mb-5'>
                        {/* Line */}
                        <div className='h-full border-l-2 border-slate-500'></div>
                        <div className='flex flex-col gap-5'>
                            {/* project Details */}
                            <div className='text-slate-400 relative flex flex-col items-start gap-4 mb-4'>
                                <h1 className='text-xl text-white font-bold'>{project.name}</h1>
                                <div className='relative group'>
                                    <div className='absolute group-hover:h-1/2 transition-all duration-200 bottom-0 h-0 left-0 px-4  text-white w-full overflow-y-scroll bg-black  rounded-md hide-scrollbar'> {/* Works */}
